...
/* edge interrupt */
#define HAL_KEY_SW_6_EDGEBIT  BV(0) //P0ICON
...
/* edge interrupt */
#define HAL_KEY_SW_7_EDGEBIT  BV(0) //P0ICON
...
#define HAL_KEY_SW_7_ICTLBIT  BV(4) /* P0IEN - P0.1 enable/disable bit */
...
uint8 halGetJoyKeyInput(void)
{
  ...
  do
  {
    ...
    if ((adc >= 28) && (adc <= 38))
    {
       ksave0 |= HAL_KEY_LEFT;//HAL_KEY_UP;
    }
    else if ((adc < 28))
    {
    ...
    {
      ksave0 |= HAL_KEY_DOWN;//HAL_KEY_LEFT;
    }
    else if ((adc >= 89) && (adc <= 100))
    {
      ksave0 |= HAL_KEY_UP;//HAL_KEY_CENTER;
    }
  } while (ksave0 != ksave1);
