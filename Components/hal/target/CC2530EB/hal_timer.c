...
{
    //halProcessTimer1 ();
    if((T1STAT & 0x02) && (T1STAT & 0x01))
    {
        //T1STAT = 0x00;

        //T1CC1L = IR_Encoding[ (T1_CNT1++)%IR_CNT ];
        //T1CC0L = 47;

        //T1CC1H = 0;
        //T1CC0H = 0;
    }
    //else if( T1STAT & 0x04 )  //T1.CH2 INT FLAG
    //{
    //    P1DIR |= 0x03; 
    //    P1_1=0;
    //    P1_0=0;
    ...
    //    T1STAT = 0x00;        //Clear T1 INT Flag
    //    IR_Encoding[T1_CNT1] = T1CC2L;
    //    IR_Encoding[T1_CNT1++] += T1CC2H*0x100;
    //}
    else if ( T1STAT & 0x01 ) //T1.CH0 INT,21ms
    {
        T1CNTL = 0x00;        //Clear T1.CNT
        T1STAT = 0x00;        //Clear T1 INT Flag
        //T1CTL &= ~0x03;       //Shutdown T1
        //if( (T1_CH0_OVF == 0) && (T1_CNT1 != 0 ) )
        //{
        //    IR_Encoding[T1_CNT1] = 0xFFFF;
        //    T1_CH0_OVF++;
        //}
        //else if( (T1_CNT1 != 0) && (T1_CH0_OVF ==1) )
        {
            Hal_IR_RCVD_Int_Config( HAL_IR_INTERRUPT_ENABLE );

            initUARTtest();
            IR_Encoding[0]=0x0123;
            UartTX_Send_String(IR_Encoding,T1_CNT1);
            T1_CNT1 = 0;
            //T1_CH0_OVF=0;
